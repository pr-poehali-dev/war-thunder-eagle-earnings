...
function WithdrawTab({ eagles, amount, setAmount, method, setMethod }: {
  ...
  const methods = [
    ...
    { id: "gaijin", label: "Gaijin Coins", icon: "Coins" },
    { id: "card", label: "Банковская карта", icon: "CreditCard" },
  ];
  const num = parseInt(amount) || 0;
  const canWithdraw = num >= 1000 && num <= eagles;
  return (
    <div>
      ...
      <div className="mt-3 p-4 bg-yellow-600/10 border border-yellow-600/30 rounded-sm animate-fade-in-up">
        ...
        <div className="text-xs text-muted-foreground mt-1">Минимальная сумма вывода: 1 000 орлов</div>
      </div>
      ...
      <div className="mt-4 animate-fade-in-up delay-200">
        ...
        {num > 0 && num < 1000 && (
          <div className="text-xs text-red-400 mt-1 flex items-center gap-1">
            <Icon name="AlertCircle" size={12} /> Минимум 1 000 орлов
          </div>
        )}
      </div>
      <button
        disabled={!canWithdraw}
        className={`mt-4 w-full py-3 font-oswald text-sm tracking-widest rounded-sm border transition-all animate-fade-in-up delay-300 ${canWithdraw ? "bg-yellow-600 border-yellow-500 text-black hover:bg-yellow-500" : "bg-muted/20 border-border text-muted-foreground cursor-not-allowed"}`}
      >
        ВЫВЕСТИ ОРЛОВ
      </button>
    </div>
